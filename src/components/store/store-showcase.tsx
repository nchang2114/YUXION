...
export function StoreShowcase({ items }: { items: ProductItem[] }) {
    ...
    <div className={cn(styles.container)}>
      ...
      <div className="relative">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3 border-b border-border/60 pb-4">
          <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">Browse Products</p>
          <p className="text-xs text-muted-foreground">
            {items.length} products in {categoryCount} categories
          </p>
        </div>
        <StoreFilterGrid items={items} filter={filter} onFilterChange={setFilter} />
      </div>
    </div>
  );
}
